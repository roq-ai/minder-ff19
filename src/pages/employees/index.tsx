...
function EmployeeListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<EmployeeInterface[]>(
    ...
    () =>
      getEmployees({
        relations: ['user', 'company', 'water_intake.count'],
      }),
  );
        ...
        {isLoading ? (
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>daily_water_goal</Th>
                  <Th>reminder_frequency</Th>
                  <Th>reminder_start_time</Th>
                  <Th>reminder_end_time</Th>
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>user</Th>}
                  {hasAccess('company', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>company</Th>}
                  {hasAccess('water_intake', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>water_intake</Th>
                  )}
                  <Th>Actions</Th>
                </Tr>
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    <Td>{record.daily_water_goal}</Td>
                    <Td>{record.reminder_frequency}</Td>
                    <Td>{record.reminder_start_time as unknown as string}</Td>
                    <Td>{record.reminder_end_time as unknown as string}</Td>
                    ...
                    {hasAccess('water_intake', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.water_intake}</Td>
                    )}
                    <Td>
