...
import { deleteWaterIntakeById } from 'apiSdk/water-intakes';
...
function EmployeeViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<EmployeeInterface>(
    ...
    () =>
      getEmployeeById(id, {
        relations: ['user', 'company', 'water_intake'],
      }),
  );

  const water_intakeHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
          ...
          <>
            <Stack direction="column" spacing={2} mb={4}>
              ...
              <Flex alignItems="center">
                ...
                <Text fontSize="md" as="span" ml={3}>
                  {data?.reminder_start_time as unknown as string}
                </Text>
              </Flex>
              <Flex alignItems="center">
                <Text fontSize="lg" fontWeight="bold" as="span">
                ...
                <Text fontSize="md" as="span" ml={3}>
                  {data?.reminder_end_time as unknown as string}
                </Text>
              </Flex>
              <Flex alignItems="center">
                <Text fontSize="lg" fontWeight="bold" as="span">
              ...
              {hasAccess('water_intake', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/water-intakes/create?employee_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.water_intake?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/water-intakes/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.amount}</Td>
                            ...
                            <Td>
                              {hasAccess('water_intake', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/water-intakes/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('water_intake', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    water_intakeHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
